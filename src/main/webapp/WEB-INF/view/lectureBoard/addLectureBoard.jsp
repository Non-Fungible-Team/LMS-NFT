...
<style type="text/css">textarea{width: 100%; height: 1000em; border: none; resize: none;}</style>
	...
	<script>
		...
		$('document').ready(function(){ 
			...
			$('#addLectureFileUpload').click(function(){
				$('.lectureFileList').each(function(){
					if($(this).val() == ''){
						flag = false;	
					}
				});
				...
				if(flag){
					...
					$('#fileSection').append("<label class='input-group-btn' ><span class='btn btn-outline-info btn-rounded'><span  id ="+label+">파일 입력</span><input type='file' id = "+fileBtn+" class='lectureFileList' style='display: none;' name='lectureFileList' ></span></label>");
					...
					alert('첨부되지 않은 lectureFileList가 존재합니다.');
				}
				
			});
			
			//유효성검사
			$('#uploadLectureBoard').click(function(){
					$('#lectureBoardTitleHelper').text('');
					$('#lectureBoardContentHelper').text('');
					
				if($('#lectureBoardTitle').val() == ''){
					$('#lectureBoardTitleHelper').text('제목을 입력하세요');
					$('#lectureBoardTitle').focus();
				} else if($('#lectureBoardContent').val() == ''){
					$('#lectureBoardContentHelper').text('내용을 입력하세요');
					$('#lectureBoardContent').focus();
					...
					if(flag){
						$('#addLectureBoardForm').submit();
					} else {
						$('#addLectureFileUploadHelper').text('파일을 첨부해 주세요');	
						$('#addLectureFileUpload').focus();
					}
				}
			});
		});
	...
	<div class="page-wrapper">
		...
		<div class="row">
			<div class="col-lg-12 col-md-12">
			    <div class="card">
			        <div class="card-body">
			            <h4 class="card-title">강의게시글 입력</h4>
				            <div>
			            	...
			            	<form id="addLectureBoardForm" action="${pageContext.request.contextPath}/teacher/lectureBoard/addLectureBoard" method="post" enctype="multipart/form-data">
				            	<table id="zero_config" class="table table-striped table-bordered no-wrap">
									...
									<tr>
						    			<th>제목</th>
						    			<td>
						    				<input type="text" name="lectureBoardTitle" id="lectureBoardTitle" class="form-control">
						    				<span id="lectureBoardTitleHelper" class="helper"></span>
						    			</td>
						    		</tr>
						    		<tr>
						    			<th>작성자</th>
						    		...
						    		<tr>
						    			<th>내용</th>
						    			<td height= 500px>
						    				<textarea rows="20" Cols="10" name="lectureBoardContent" id="lectureBoardContent" class="form-control" placeholder="내용을 입력해 주세요."></textarea>
						    				<span id="lectureBoardContentHelper" class="helper"></span>
						    			</td>
						    		</tr>
				            	</table>

				            	<div>
				            		<button type="button" id = "addLectureFileUpload" class="btn btn-outline-info btn-rounded">파일 업로드 추가</button>
				            		<div id = "fileSection">
				            			<!-- 파일 업로드 input 태그가 추가될 영역 -->
				            		</div>
				            		<span id="addLectureFileUploadHelper" class="helper"></span>
				            	</div>
				            	<div>
				            		<button type="button" id="uploadLectureBoard" class="btn btn-outline-success btn-rounded">등록</button>
					        	</div>
				            </form>
			             </div>   
			        </div>
